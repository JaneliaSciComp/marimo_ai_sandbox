#!/usr/bin/env python3
"""Tiny TCP-to-Unix-socket relay. Runs INSIDE the Podman container.

Listens on 127.0.0.1:<port> and forwards every byte to a Unix domain socket
that was bind-mounted into the container (the other end of that socket is
allowlist_proxy.py, running outside the container with real network access).
This lets any ordinary HTTP client use plain http_proxy/https_proxy env vars
pointed at 127.0.0.1, instead of needing unix-socket-proxy support
(inconsistent across tools). Vendored from
JaneliaScientificComputingSystems/agentic-sandbox (scripts/relay.py).
