...
Runs OUTSIDE the Podman container, with real network access, listening on a
Unix domain socket (not a TCP port) so it can be bind-mounted into exactly
one container at a time. Host-side half of the Podman network-allowlist
pattern -- see the main README's Podman network section. Vendored from
JaneliaScientificComputingSystems/agentic-sandbox (scripts/allowlist_proxy.py).
